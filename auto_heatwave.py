...
update_file = start_file
start_index = na_input_files2.index(start_file)
for file in na_input_files2[start_index:]:
    if file not in os.listdir('F:/heatwave_data'):
        print(file.split('.')[0]+'未更新')
        date_object = datetime.strptime(file.split('.')[0], '%Y-%m-%d').date()
        date_object - timedelta(days=4)
        update_file = f'{date_object}.csv'
        break
if update_file == start_file:
    print("数据暂未更新")
else:
    start_index = na_input_files2.index(update_file)
    for file in na_input_files2[start_index:]:
        date_object = datetime.strptime(file.split('.')[0], '%Y-%m-%d').date()
        # 获取该日期是该年的第几天
        ...
        if initial_arrays.__len__() < 5:
            ...
            sst.to_csv(f'F:/heatwave_data/{date_str}.csv', header=False, index=False)
            sea_temperature = sst
